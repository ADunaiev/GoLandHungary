...
import { ExpenseRateTable, SupplierInvoicesTable } from "../definitions";
...
export async function fetchSupplierInvoiceById(id: string) {
  try {
    const data = await sql<SupplierInvoicesTable>`
      SELECT
        si.id,
        si.amount,
        si.date,
    ...
    console.error('Database Error:', error);
    throw new Error('Failed to fetch supplier invoice by id.');
  }
}