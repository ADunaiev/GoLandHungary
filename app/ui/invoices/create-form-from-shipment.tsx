...
import { CustomerField, CurrencyField, CustomerAgreementField, OrganisationField, RateTable, InvoiceTypeFull, CurrencyRateField, ShipmentTypeFull, RouteFullType, UnitField, ShipmentRouteUnitTypeFull } from '@/app/lib/definitions';
...
import { createInvoice, createInvoiceFromShipment, State } from '@/app/lib/actions';
...
export default function CreateInvoiceFromShipmentForm({ 
  currencies,
  agreements,
  rates,
  invoice,
  ...
  const createInvoiceWithShipment = createInvoiceFromShipment.bind(null, shipment.id, invoice.number);
  ...
  function createInvoiceRemarks(routes: RouteFullType[]) {
    let remarks = '';
      ...
      if(rates.find(rate => rate.route_id === route.id)) {
        ...
        units.map((unit) => {
          if(unit.route_id === route.id) {
            remarks += unit.vehicle_number + '/' + unit.number + ', '
          }
        })
        remarks += '\n';
      }
  ...
  const onSubmit = useDebouncedCallback( async (data: InvoiceType) => {
    try{
      await createInvoiceWithShipment(data)
    } catch (e) {
      console.log(e);
    }
  }, 300);
  ...
  return (
    <form onSubmit={(e) => {
      ...
      <div className="rounded-md bg-gray-50 p-4 md:p-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="mb-4">
          ...
          <div className="relative">
            ...
            >
              <option key={shipment.organisation_id} value={shipment.organisation_id}>
                {shipment.organisation_name_eng}
              </option>
            </select>
        ...
        <div className="mb-4">
          ...
          <div className="relative">
            ...
            >
              <option value={shipment.customer_id} key={shipment.customer_id}>
                {shipment.customer_name_eng}
              </option>
            </select>
        ...
        <div className="mb-4">
          ...
          <div className="relative">
            <textarea
                  ...
                  defaultValue={createInvoiceRemarks(routes)}
                />
            <DocumentTextIcon className="pointer-events-none absolute left-3 top-1/2 h-[18px] w-[18px] -translate-y-1/2 text-gray-500" />
          </div>
        </div>
      ...
      <div className="mt-6 flex justify-end gap-4">
        <Link
          href={`/dashboard/shipments/${shipment.id}/edit?tab=4`}
