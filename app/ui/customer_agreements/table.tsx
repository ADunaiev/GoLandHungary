...
export default async function CustomersAgreementsTable({ query, currentPage } : {
    query: string,
    currentPage: number,
}) {
    const agreements = await fetchFilteredCustomersAgreements(query, currentPage);
                  ...
                  >
                    <div className="flex items-center justify-between border-b pb-4">
                        ...
                        <div key='buttons' className="flex justify-end gap-2">
                          <UpdateCustomerAgreement key='mob_update_btn' id={agreement.id} />
                          <DeleteCustomerAgreement key='mob_delete_btn' id={agreement.id} />
                        </div>
                    </div>
                    ...
                    >
                      ...
                      <td key='agreement_buttons_md' className="whitespace-nowrap py-3 pl-6 pr-3">
                        <div className="flex justify-end gap-3">
                          <UpdateCustomerAgreement key='update_btn' id={agreement.id} />
                          <DeleteCustomerAgreement key='delete_btn' id={agreement.id} />
                        </div>
                      </td>
                    </tr>
                  ))}
